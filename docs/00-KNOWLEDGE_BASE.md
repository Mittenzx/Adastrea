...
- **`ASpaceshipInterior`**: walkable room actor. `ConfigureInterior(mesh)` loads the shell mesh (explicit arg → `DefaultInteriorMesh` → C++ fallback `SM_Int_Fighter_EmptyRoom`), **normalizes scale to TargetRadius 650**, fits the walk volume + seat/exit trigger to mesh bounds, mounts per-family companion parts, assigns `M_Int_*` materials by slot name. Ambient fill + 2 warm fixture point-lights on reveal (no-Lumen plan).
- **`USpaceshipDataAsset`**: per-ship tuning (hull, cargo, mobility, combat, lore) + **Avatar Spawn** override (`bUseCustomAvatarSpawn`, `AvatarSpawnOffset` ±Yaw) used by `EnterShipInterior`. 27 authored ship data assets in `Content/DataAssets/Ships/`.
- **Per-ship interior wiring**: `BP_Ship_*` Blueprints carry `interiorShellMesh` (soft ref). Fighter currently → `SM_Int_Fighter_EmptyRoom` (set via MCP on `BP_Ship_Fighter.Default__BP_Ship_Fighter_C`, saved 2026-09-13).
...
| Script | Purpose |
|---|---|
| `render_studio.py` | Whole-ship studio shots (PARTS_ORDER + MOUNT_FRAC parity) |
| `render_material_preview.py` | Close-up flat tiling plane — the honest way to judge textures |
...
| `hand_render.py` | Hand-authored ship renders |
