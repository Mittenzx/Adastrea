    ...
    for header_file in header_files:
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                # Check if this line or nearby lines have UPROPERTY
                has_uproperty = False
                # Check current line and previous 3 lines for UPROPERTY
                for check_line in range(max(1, line_num - 3), min(len(lines) + 1, line_num + 1)):
                    if 'UPROPERTY' in lines[check_line - 1]:
                        has_uproperty = True
                        break
