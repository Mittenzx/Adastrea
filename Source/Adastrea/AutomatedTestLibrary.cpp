...
bool UAutomatedTestLibrary::TestWeaponDamage(UObject* WorldContextObject, FTestResult& OutResult)
{
    return ExecuteTest([&](FTestResult& Result) -> bool {
        ...
        if (Weapons.Num() == 0)
        {
            Result.Message = TEXT("No weapon data assets found");
            return false;
        }
        ...
        for (UWeaponDataAsset* Weapon : Weapons)
        {
            ...
            if (BaseDamage < 0)
            {
                Result.Errors.Add(FString::Printf(TEXT("Weapon %s has negative base damage"),
                    *Weapon->WeaponName.ToString()));
                return false;
            }

            // Test damage with armor
            float ArmorMitigated = UAdastreaFunctionLibrary::CalculateDamageAfterArmor(
                BaseDamage, 50.0f, EDamageType::Kinetic);

            if (ArmorMitigated > BaseDamage)
            {
                Result.Errors.Add(TEXT("Armor mitigation should not increase damage"));
                return false;
            }

            // Test firing arc
            bool bInArc = UAdastreaFunctionLibrary::IsInFiringArc(
                FVector::ZeroVector, FRotator::ZeroRotator,
                FVector(100, 0, 0), 45.0f);

            // Should be in arc for forward firing
            if (!bInArc)
            {
                Result.Errors.Add(TEXT("Forward target should be in firing arc"));
                return false;
            }

            // Test time to impact
            float TimeToImpact = UAdastreaFunctionLibrary::CalculateTimeToImpact(1000.0f, 500.0f);
            if (TimeToImpact <= 0)
            {
                Result.Errors.Add(TEXT("Time to impact should be positive"));
                return false;
            }
        }
