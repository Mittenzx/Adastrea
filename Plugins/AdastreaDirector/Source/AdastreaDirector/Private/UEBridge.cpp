...
#if WITH_EDITOR
	// TODO: Fix for UE5.6 - GetSelectedAssets method not found
...
FAdastreaResult UUEBridge::LoadLevel(const FString& LevelPath)
{
	...
	FURL URL(nullptr, *LevelPath, TRAVEL_Absolute);
	FString Error;
...
FAdastreaResult UUEBridge::SaveCurrentLevel()
{
	...
	bool bSaved = EditorAssetSubsystem->SaveAsset(World->GetCurrentLevel()->GetPathName(), false);
	if (bSaved)
	{
